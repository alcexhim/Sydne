<?php
	...
	use Sydne\Objects\Refund;
	use Sydne\Objects\RefundProduct;
	
	use Sydne\Controls\Keypad;
	use Sydne\Controls\KeypadCharacterSet;
	
	class RefundListPage extends SydnePage
	{
		protected function RenderContent()
		{
			$Refunds = Refund::Get();
			
			$lvRefunds = new ListView();
			$lvRefunds->Columns = array
			(
			...
			foreach ($Refunds as $Refund)
			{
				...
				(
					new ListViewItemColumn("chTimestampOrdered", $Refund->TimestampOrdered),
					// new ListViewItemColumn("chCustomer", "Customer"),
					new ListViewItemColumn("chEmployee", $Refund->Employee->DisplayName),
					new ListViewItemColumn("chAmountPaid", $Refund->PaymentAmount),
					new ListViewItemColumn("chPaymentType", $Refund->PaymentType->Title)
				));
				$lvi->NavigateURL = "~/Refunds/Detail/" . $Refund->ID;
				$lvRefunds->Items[] = $lvi;
			}
	...
	class RefundDetailPage extends SydnePage
	{
		public $Refund;
		protected function Initialize()
		{
			parent::Initialize();
			$this->ToolbarRightItems[] = new ButtonGroupButton("btnReturnToRefunds", "Refunds List", null, null, "~/Refunds");
		}
		...
		?>
			<div class="Receipt" style="width: 400px; margin-left: auto; margin-right: auto; height: auto; border: inset 1px; padding: 8px;">
				<table style="width: 100%;">
					...
					<?php
					$products = $this->Refund->GetProducts();
					$TotalCost = 0.00;
					...
					<tr>
						...
						<td><span style="padding-right: 8px;">$</span><?php
							echo(number_format($this->Refund->PaymentAmount, 2));
						?></td>
					</tr>
					<tr>
						<td colspan="2" style="font-size: 16pt; font-weight: bold;">Change</td>
						<td style="font-size: 14pt; font-weight: bold;"><span style="padding-right: 8px;">$</span><?php echo(number_format(floatval($this->Refund->PaymentAmount) - floatval($TotalCost), 2)); ?></td>
					</tr>
				</table>
			</div>
<?php
	...
	class RefundModifyPage extends SydnePage
	{
		protected function Initialize()
		{
			parent::Initialize();
			...
			$this->Variables[] = new WebPageVariable("IsRefundFinalized", "false");
			$this->Variables[] = new WebPageVariable("CustomerPaymentAmount", "0.00");
			
			$this->Title = "Make a Refund";
		...
		protected function AfterVariablesInitialize()
		{
			...
			else if ($this->GetVariableValue("IsRefundFinalized") != "true")
			{
			...
			if (!$this->IsVariableSet("CustomerPaymentAmount"))
			{
				if (!$this->IsVariableSet("CustomerID"))
				{
					$this->ToolbarRightItems[] = new ButtonGroupButton("btngCancelRefund", "Main Menu", null, "~/Images/Buttons/Return.png", "~/");
				}
				else
				{
					$this->ToolbarRightItems[] = new ButtonGroupButton("btngCancelRefund", "Cancel Refund", null, "~/Images/Buttons/Cancel.png", "~/Refunds/Create");
				}
			}
			else
			{
				$this->ToolbarRightItems[] = new ButtonGroupButton("btngNewRefund", "New Refund", null, "~/Images/Buttons/RefundCreate.png", "~/Refunds/Create");
			...
			else if ($this->GetVariableValue("IsRefundFinalized") != "true")
			{
				...
				$this->ToolbarLeftItems[] = new ButtonGroupButton("btngProductEntry_FinalizeRefund", "Finalize Refund", null, "~/Images/Buttons/Accept.png", null, "cmdProductEntryFinalizeRefund_Click();");
		...
		<script type="text/javascript">
			...
			function RefreshReceipt()
			{
				...
				Sydne.ExternalDisplay.RefundDetail(text);
		...
		<table style="width: 100%">
			<tr>
				...
				<td style="vertical-align: top; width: 300px;">
					<div class="Window">
						<div class="TitleBar"><span class="Text">Current Refund</span></div>
						<div class="Content" style="padding: 4px; width: 300px;">
							<div class="Receipt" style="width: 100%;">
								...
								<script type="text/javascript">
									Sydne.ExternalDisplay.RefundDetail("<?php echo($totalText); ?>");
								</script>
		...
		{
			WebPage.ClearVariableValue("IsRefundFinalized");
		}
		</script>
		<?php
			...
			else if (!$this->IsVariableSet("CustomerPaymentAmount"))
			{
						...
						<td colspan="3">
							<span style="background-color: #FFFFFF; padding: 8px; font-weight: bold; width: 100%; text-align: right; display: block;"><span style="padding-right: 8px; font-size: 18pt;">Total: $</span><span style="font-size: 18pt;"><?php
								...
								for ($i = 0; $i < $count; $i++)
								{
									...
									$Refund->Products[] = new RefundProduct($Refund, $product, $quantity);
				...
				<script type="text/javascript">
					...
					{
						return "A Refund is still in progress. Do you wish to exit?";
					};
				</script>
				<?php
			}
			else
			{
				...
				$Refund = new Refund();
				$Refund->Employee = $CurrentEmployee;
				$Refund->TimestampOrdered = date();
				$Refund->TimestampPaid = date();
				$Refund->PaymentType = PaymentType::GetByID($this->GetVariableValue("PaymentTypeID"));
				$Refund->PaymentAmount = $this->GetVariableValue("CustomerPaymentAmount");
				...
				<div class="Receipt" style="width: 400px; margin-left: auto; margin-right: auto; height: auto; border: inset 1px; padding: 8px;">
					...
					<script type="text/javascript">
						Sydne.ExternalDisplay.RefundFinal(<?php echo($Subtotal); ?>, <?php echo($Tax); ?>, <?php echo($CustomerPayment); ?>);
